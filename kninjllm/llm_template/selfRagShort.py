...
class selfRagShortTemplate:
    ...
    def run(self,
        ...
        for index,query in enumerate(query_list):
            res_ctxs = self.retriever.run(query=query['question'])['final_result'][0]
            query_list[index]['ctxs'] = res_ctxs
    ...
    def call_model_rerank_w_scores_batch(self,prompt, evidences, max_new_tokens=15,
        ...
        if mode != "always_retrieve":
            ...
            pred = self.generator.run(prompt=prompt, sampling_params=sampling_params,saveLogFlag=False)['final_result'][0]['meta']['pred']
        ...
        if do_retrieve is True:
            evidence_augmented_inputs = [prompt + "[Retrieval]<paragraph>{0}\n{1}</paragraph>".format(
                para.get('title',''), para["content"]) for para in evidences]
                
            sampling_params = SamplingParams(
                temperature=0.0, top_p=1.0, max_tokens=max_new_tokens, logprobs=5000)
            preds = []
            for input_i in evidence_augmented_inputs:
                pred = self.generator.run(prompt = input_i, sampling_params=sampling_params,saveLogFlag=False)['final_result'][0]['meta']['pred']
                preds.append(pred)
            ...
            for one_prompt in [prompt]:
                pred = self.generator.run(prompt=one_prompt, sampling_params=sampling_params)['final_result'][0]['meta']['pred']
                preds.append(pred)
