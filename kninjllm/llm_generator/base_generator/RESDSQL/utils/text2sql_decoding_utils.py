...
def decode_natsqls(
    ...
    mysql_utils = MySQLUtils(host=dbInfo['host'], user=dbInfo['user'], password=dbInfo['password'], database=dbInfo['database'])
    ...
    for batch_id in range(batch_size):
        ...
        for seq_id in range(num_return_sequences):
            ...
            if old_pred_natsql != pred_natsql:
                ...
                print("---------------")
            pred_sql = natsql_to_sql(pred_natsql, db_id, db_file_path, table_dict[db_id],root_path,dbInfo).strip()
            ...
            try:
                ...
                mysql_utils.execute_query(pred_sql)
