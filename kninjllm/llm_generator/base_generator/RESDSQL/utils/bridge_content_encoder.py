...
def get_column_picklist(table_name: str, column_name: str, db_path: str,root_path:str,dbInfo:str) -> list:
    
    import sys
    sys.path.append(root_path + "kninjllm/llm_knowledgeUploader/utils")
    ...
    try:
        ...
        conn = MySQLUtils(host=dbInfo['host'], user=dbInfo['user'], password=dbInfo['password'], database=dbInfo['database']).connection
...
def get_database_matches(
    ...
    picklist = get_column_picklist(
        table_name=table_name, column_name=column_name, db_path=db_path,root_path=root_path,dbInfo=json.dumps(dbInfo,ensure_ascii=False)
    )
