...
def construct_hyper_param():
    ...
    parser.add_argument('--root_path', default='/app/', type=str)
    parser.add_argument('--dbInfo', default="", type=str)
    args = parser.parse_args()
    if isinstance(args.dbInfo,str):
        args.dbInfo = json.loads(args.dbInfo)
...
def re_identify_boolean_type(tables,use_extra_col_types,database_path,root_path,dbInfo):
    for db_i,table in enumerate(tables):
        print()
        print(tables[db_i]['db_id'] + " " + str(db_i))
        try:
            db_ = DBEngine(table,database_path,root_path,dbInfo)
        except:
            continue
...
def analyse_same_column(tables,schemas,database_path,root_path,dbInfo):
...
def correct_primary_keys(tables,schemas,database_path,root_path,dbInfo):
    for it, table,schema in zip(range(len(tables)),tables,schemas):
...
def label_disjoint_tables(tables,database_path,root_path,dbInfo):
...
if __name__ == '__main__':
    ...
    if args.correct_primary_keys:
        correct_primary_keys(tables,schemas,database_path,args.root_path,args.dbInfo)
        label_disjoint_tables(tables,database_path,args.root_path,args.dbInfo)
    
    if args.correct_col_type:
        tables = re_identify_boolean_type(tables,args.use_extra_col_types,database_path,args.root_path,args.dbInfo)
    ...
    if args.analyse_same_column:
        tables = analyse_same_column(tables,schemas,database_path,args.root_path,args.dbInfo)
