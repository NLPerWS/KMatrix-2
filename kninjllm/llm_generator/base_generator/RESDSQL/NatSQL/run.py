...
def construct_hyper_param():
    ...
    parser.add_argument('--root_path', default='/app/')
...
if __name__ == '__main__':
    ...
    for i,sql in enumerate(sqls):
        if "pattern_tok" in sql:
            ...
            sq = None
        try:
            query,_,__ = create_sql_from_natSQL(sql["NatSQL"], sql['db_id'], "data/spider/database/"+sql['db_id']+"/"+sql['db_id']+".sqlite", 
                                                table_dict[sql['db_id']], sq, remove_values=args.test_executable_natsql, 
                                                remove_groupby_from_natsql=args.remove_groupby_from_natsql,
