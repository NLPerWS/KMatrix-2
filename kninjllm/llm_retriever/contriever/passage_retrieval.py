...
import kninjllm.llm_retriever.contriever.src.index
import kninjllm.llm_retriever.contriever.src.contriever
import kninjllm.llm_retriever.contriever.src.utils
import kninjllm.llm_retriever.contriever.src.slurm
import kninjllm.llm_retriever.contriever.src.data
from kninjllm.llm_retriever.contriever.src.evaluation import calculate_matches
import kninjllm.llm_retriever.contriever.src.normalize_text
    ...
    with torch.no_grad():
        ...
        for k, q in enumerate(queries):
            ...
            if args.normalize_text:
                q = kninjllm.llm_retriever.contriever.src.normalize_text.normalize(q)
...
def main_bak(args):
    kninjllm.llm_retriever.contriever.src.slurm.init_distributed_mode(args)

    print(f"Loading model from: {args.model_name_or_path}")
    model, tokenizer, _ = kninjllm.llm_retriever.contriever.src.contriever.load_retriever(args.model_name_or_path)
    model.eval()
    model = model.cuda()
    if not args.no_fp16:
        model = model.half()

    index = kninjllm.llm_retriever.contriever.src.index.Indexer(args.projection_size, args.n_subquantizers, args.n_bits)

    
    passages = kninjllm.llm_retriever.contriever.src.data.load_passages(args.passages)
...
def main_infer(args):
    ...
    if len(args.passages) > 0:
        ...
        allembeddings = []
        for i, doc in enumerate(passages):
            if 'contriever_embedding' not in doc or doc['contriever_embedding'] == None:
        ...
        # print(dir_list)
        # print("---------------------filter km2_rule-------------------------")
        # dir_list = list(filter(lambda x:not "km2_rule" in  x,dir_list))
        # print(dir_list)
        
        for file_index,fileName in enumerate(dir_list):
            print(f"--------------------retriever by {fileName}--- {file_index+1}/{len(dir_list)} ---------------")
